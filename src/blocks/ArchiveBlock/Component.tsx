import type { Post, ArchiveBlock as ArchiveBlockProps, Page, Member, Newsletter } from '@/payload-types'
    ...
    if (selectedDocs?.length) {
      const filteredSelectedPosts = selectedDocs.map((post) => {
        if (typeof post.value === 'object')
          return { ...post.value, relationTo: post.relationTo || 'posts' }
      }) as (
        | (Post & {
            relationTo: 'posts' | 'members' | 'pages' |'newsletters' | undefined
          })
    ...
    <div className="py-16 bg-amber-50" id={`block-${id}`}>
      {introContent && (
        <div className="container mb-16 text-brand-blueBright text-center">
          <RichText
            className="mb-4 max-w-prose px-14 text-inherit prose-xl prose-h2:text-7xl prose-h2:mb-4 prose-h2:font-display prose-h2:font-bold"
